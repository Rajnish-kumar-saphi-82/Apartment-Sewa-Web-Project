...
export class DashboardController {
  ...
  async createNotice(req: Request, res: Response) {
    try {
      ...
      const notice = await dashboardService.createNotice(noticeData);
  ...
  async createUnit(req: Request, res: Response) {
    try {
      const data = CreateUnitDTO.parse(req.body);
      if (req.file) {
  ...
  async updateUnitStatus(req: Request, res: Response) {
    try {
      ...
      const unit = await dashboardService.updateUnitStatus(id as string, data);
  ...
  async createTenant(req: Request, res: Response) {
    try {
      ...
      const tenant = await dashboardService.createTenant(data);
  ...
  async getMyBills(req: Request, res: Response) {
    try {
      ...
      if (!authUser?.phone && !authUser?.full_name) return ApiResponseHelper.success(res, [], "No bills found");
  ...
  async createBill(req: Request, res: Response) {
    try {
      ...
      const bill = await dashboardService.createBill(data);
  ...
  async payBill(req: Request, res: Response) {
    try {
      ...
      const bill = await dashboardService.payBill(id, data);
      return ApiResponseHelper.success(res, bill, "Bill paid successfully");
    } catch (error: any) {
      console.error("[payBill] ERROR:", error);
      console.error("[payBill] Error message:", error?.message);
      console.error("[payBill] Error stack:", error?.stack);
      if (error.errors) return ApiResponseHelper.validationError(res, error.errors);
  ...
  async createTicket(req: Request, res: Response) {
    try {
      ...
      if (req.file) {
        ...
        // We pass the local path to the file for reading in the service
        diagnostic = await analyzeMaintenanceImage(req.file.path);
      }
      
      const ticket = await dashboardService.createTicket({ ...(data as any), diagnostic });
  ...
  async updateTicketStatus(req: Request, res: Response) {
    try {
      ...
      if (!parsedData.success) {
        return ApiResponseHelper.error(res, parsedData.error.issues[0].message, 400, parsedData.error.issues);
      }
      const ticket = await dashboardService.updateTicketStatus(req.params.id as string, parsedData.data);
  ...
  async getAnalytics(req: Request, res: Response) {
    try {
      ...
      const occupiedUnits = await UnitModel.countDocuments({ status: "Occupied" });
      const vacantUnits = await UnitModel.countDocuments({ status: "Vacant" });
      return res.status(200).json({
        success: true,
        data: {
          revenue: revenueData.map(d => ({ month: d._id, revenue: d.total })),
          occupancy: [
